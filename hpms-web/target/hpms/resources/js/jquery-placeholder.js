$.fn.placeholder = function(){
    return this.each(function () {
        if(Modernizr.input["placeholder"] == false){
            ...
            var color = '#777777';
            //div setup
            var placeholder = $('<div class="place_holder">'+phv+'</div>');
            $('body').append(placeholder);
            var top = _self.offset().top + ((_self.outerHeight() - _self.height()) / 2) + 8;
            var left = _self.offset().left + ((_self.outerWidth() - _self.width()) / 2) + 26;
